...
Route::get('/test-ip', function(GeoIPService $geo){
    return $geo->isBlocked(request()->ip())
        ? 'IP bloqueada'
        : 'IP permitida';
});
...
Route::get('verify/account/{confirmation_code}', [HomeController::class, 'getVerifyAccount'])->where('confirmation_code','[A-Za-z0-9]+');
 ...
 Route::group(['middleware' => 'auth'], function() {
	...
	// Video Call URL
	Route::get('video/call/{token}', [VideoCallController::class, 'videoCallUrl'])->name('video.call.url');
