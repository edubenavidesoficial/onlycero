...
class UserController extends Controller
{
  ...
  public function profile($slug, $media = null)
  {
    ...
    // Stripe Key
      ...
      if (auth()->check() && config('settings.hide_admin_profile') == 'on'
          && $user->id == 1
          && auth()->id() != 1
          ) {
            abort(404);
  ...
  public function postDetail($slug, $id)
  {
    ...
    $updatesCount = $user->updates->count();
      ...
      if (auth()->check() && $updatesCount != 0 && $user->updates[0]->user_id != auth()->id()
        && $user->updates[0]->status == 'pending'
        && auth()->user()->role != 'admin'
      ) {
  			abort(404);
  		} elseif (auth()->guest() && $updatesCount != 0 && $user->updates[0]->status == 'pending') {
  			abort(404);
  		}
      ...
      //<<<-- * Redirect the user real name * -->>>
      $uri = request()->path();
      $uriCanonical = $user->username.'/post/'.$user->updates[0]->id;
  ...
  protected function createPlanPaystack()
  {
   ...
   $path = config('path.cover');
    ...
    public function myPosts()
    {
      ...
      ->withCount(['likes', 'bookmarks', 'comments', 'replies'])
      ->where('status', '<>', 'encode')
      ->orderBy('id', 'desc')
      ->paginate(15);
