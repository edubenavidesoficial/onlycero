          ...
          <form action="{{ url()->current() }}" method="post" enctype="multipart/form-data" id="addStoryForm">
            ...
            <div class="my-3 container-backgrounds d-block">
              @foreach ($storyBackgrounds as $background)
                <img src="{{ url('/imgstories-bg', $background->name) }}" data-bg-name="{{ $background->name }}" data-bg="{{ url('/imgstories-bg', $background->name) }}" class="mr-1 mb-2 storyBackgrounds storyBg">
            @endforeach
            </div>
        ...
        <div class="col-lg-7 first">
          <div class="d-block w-100">
            <div class="bg-current w-100 bg-black mb-3 d-block py-4 px-lg-0 px-4">
              <div class="bg-inside text-center mx-auto" style="background: #6a6a6a url('{{ url('/imgstories-bg', $storyBackgrounds[0]['name']) }}') no-repeat center center; background-size: cover;">
